...
def get_data(name):
    with open('./text/{}'.format(name), encoding='utf-8') as f:
        data = json.load(f)
    return data
...
class In_trust_Loss(nn.Module):
    def __init__(self, alpha=1, beta=0.8, delta=0.5, num_classes=1800):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
...
from models import MutualSL

model = MutualSL.from_pretrained(CFG['model'])
...
for epoch in range(CFG['epochs']):
    ...
    for i in range(len(token_id)):
        ...
        iou = calculate_iou(i0=[start_time, end_time],
                            i1=[valid[i]["answer_start_second"], valid[i]["answer_end_second"]])
        ious.append(iou)
    ...
    log(score_str, path)
    val_loss, pred_start, pred_end, token_id = test_model(model, test_loader, epoch)
    ious = []
    for i in range(len(token_id)):
        start = pred_start[i].argmax().item()
        end = pred_end[i].argmax().item()
        ...
        for t_num in range(len(token)):
            ...
            if t_num == end:
                end_time = test[i]['video_sub_title'][len(tokens) - 1]['start'] + \
                           test[i]['video_sub_title'][len(tokens) - 1]['duration']
            if token[t_num] == 2:
                tokens.append(ts)
                ts = []
        try:
            ...
            start_time = test[i]['video_sub_title'][0]['start']
            end_time = test[i]['video_sub_title'][-1]['start'] + test[i]['video_sub_title'][-1]['duration']
        iou = calculate_iou(i0=[start_time, end_time],
                            i1=[test[i]["answer_start_second"], test[i]["answer_end_second"]])
        ious.append(iou)
    ...
    model_name = path + '/{}_{}_{}_{}_{}_model'.format(epoch, round(r1i3_1, 2), round(r1i3_2, 2), round(mi1, 2),
                                                       round(mi2, 2))
